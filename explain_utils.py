...
# Load saved classifier + label encoder + encoder name
clf = joblib.load("xgb_ped_binary_minilm.joblib")
le = joblib.load("label_encoder_ped_binary.joblib")
with open("encoder_name.txt", "r", encoding="utf-8") as f:
    encoder_name = f.read().strip()
encoder = SentenceTransformer(encoder_name)

# Load top 50 TF-IDF+XGB features (global signals)
...
def _predict_label_and_prob(text: str) -> Dict[str, Any]:
    """Predict Good/Poor and P(Good) using the saved MiniLM+XGB model."""
    emb = encoder.encode([text], batch_size=1, show_progress_bar=False)
    probs_full = clf.predict_proba(emb)

    # Find index of "Good" in label encoder
...
def _build_explanation(label: str, matched_feats: List[str]) -> str:
    ...
    if not matched_feats:
        if label == "Good":
            return (
                "Predicted as Good mainly from overall linguistic patterns, but "
                "no top global scaffolding phrases were detected. Consider adding "
                "explicit hints or step-by-step prompts."
            )
        else:
            return (
                "Predicted as Poor and no strong scaffolding or encouragement "
                "phrases were detected. The response may be answer-only or vague."
            )
    ...
    if label == "Good":
        return (
            f"Predicted as Good because it includes helpful pedagogical phrases "
            f"such as {feats_str}, which reflect encouragement, scaffolding, or "
            f"explicit reasoning."
        )
    else:
        return (
            f"Predicted as Poor even though it contains phrases like {feats_str}. "
            f"These may be insufficiently supported by concrete guidance or next "
            f"steps for the student."
        )
...
if __name__ == "__main__":
    examples = [
        "Let's go step by step. First, we calculate the length using the formula, then see if your answer matches.",
        "The answer is 42.",
    ]
